#!/usr/bin/env python
#receive code and create an image.
...
class PlantUML(object):
    ...
    def processes_file(self, filename,code, outfile=None, errorfile=None, directory=''):
        ...
        try:
            ...
            print(f'content from plantuml : {content}')
...
def processes_file_code(self, filename, plantuml_code, outfile=None, errorfile=None, directory=''):
        """Take a filename of a file containing plantuml text and processes
        it into a .png image.
        ...
        data = plantuml_code
        data = replace_single_with_double_quotes(data)
        try:
            content = self.processes(data)
        except PlantUMLHTTPError as e:
            err = open(path.join(directory, errorfile), 'w')
...
def save_code_to_file(code):
    ...
    # Define the filename where you want to save the string
    filename = 'diagram.txt'

    # Open the file in write mode ('w') and write the string to it
    with open(filename, 'w') as file:
        file.write(code.strip())  # .strip() removes leading and trailing whitespace, including the newline at the start

    print(f"The code has been saved to '{filename}'.")
...
def create_plantuml_image(code):
    
    args = _build_parser().parse_args()
    pl = PlantUML(args.server)

     
    image = pl.processes(code)
    
    return image 
    

def create_plantuml_file(code, filename):

    args = _build_parser().parse_args()
    pl = PlantUML(args.server)

    code = replace_single_with_double_quotes(code)
    pl.processes_file(filename, code, directory=args.out)
...
if __name__ == '__main__':
    mycode = "@startuml\nleft to right direction\nactor 'Synthetic Purchase Order Generator' as SPOG\nactor 'System' as SYS\nrectangle 'Flight FET Validation' {\n  SPOG -- (Verify US Operator)\n  SYS -- (Maintain US Operators List)\n  (Verify US Operator) .> (Maintain US Operators List) : <<include>>\n  (Verify US Operator) -- (Check Departure Airport)\n  (Check Departure Airport) .> (Flag Flight as Subject to FET) : <<extend>>\n}\n@enduml"
    filename ='sample.png'
    create_plantuml_file( mycode, filename)
